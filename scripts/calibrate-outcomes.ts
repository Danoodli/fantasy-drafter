...
async function main() {
  ...
  for (const pos of SKILL) {
    ...
    // PER-GAME ratio: what he scored per game he played vs what the projection
    // implies per game. Availability is modeled separately, so a season ratio
    // here would count missed games twice.
    const ratio = skill.map((r) => Math.max(0.05, (r.season / r.played) / (r.p.projPoints / GAMES)));
  ...
  for (const pos of SKILL) {
    const g = rows.filter((r) => r.p.pos === pos);
    const observed = mean(g.map((r) => r.season / r.p.projPoints));
    const pp = byPos[pos];
    ...
    console.log(`  ${pos}: observed ${observed.toFixed(3)}   model ${(avail * eRatio).toFixed(3)}   (avail ${avail.toFixed(3)} × ratio ${eRatio.toFixed(3)})`);
  }
}
