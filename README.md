...
- **Screen sync** (any site, fully automatic). Click **Screen sync**, share the browser tab your draft is in, drag a box around its **pick history** (drafted players only — not the available list), and **Watch**. The cockpit reads that region several times a second (two Tesseract.js workers in your browser, WASM — nothing leaves the machine, no login to the draft site) and finds board names in what it read (see *How names are matched* below). **What it trusts is order, never pick numbers**: OCR junk in front of a name is not a pick number, but the panel's top-to-bottom order is exactly the pick order. Each read is reconciled with the picks already known (`lib/draft/sequence.ts`): new names append in order, a pick that was missed is inserted where it belongs and the picks after it shift down, a burst of fast picks lands in one read, and ownership follows the room's snake (slot 12 owns picks 12 and 13). A name must appear in two consecutive reads before it counts. **Your own picks are recorded too**: draft on the site — back-to-back picks included — and the app records what you took, with the same celebration as the Draft button; it advises, it never chooses. If you'd rather click Draft in the app first, that works as well (the screen read then sees a known name and moves on). ESPN lists oldest at top; tick **newest pick at top** for rooms that don't. **Draft board grids** (DraftKings, Yahoo's board view — one cell per pick, rounds snaking left and right) have no list to read in order, but every cell wears its own `round.pick` label; when two such labels are on screen the cockpit switches to grid reading (`lib/draft/ocrGrid.ts`). The labels calibrate the grid — a label's round and pick say which column it must be in, so a misread label in the wrong column is thrown out, rows are consecutive rounds, and once calibrated every cell's pick number follows from its position even when its own label came back as "5M". Each cell's words are matched line by line with the cell's position and team as hints (so `J. Williams · WR DET` is unambiguous, and "D. Montgo…" still reads); a tie the hints can't settle ("B. Robinson · RB ATL" is Bijan or Brian) goes to whoever isn't already placed elsewhere, else to the ADP that fits the pick. Snake direction never matters. Drag the box around the grid itself; a cut-off top row without labels is ignored. Frames are upscaled to ~4300 px wide before OCR (small bold labels lose their dot at 2×). The cockpit has the same board: the **Draft board** toggle above the tier board draws the room DraftKings-style (one column per slot, rounds snaking, your column marked, the pick on the clock outlined; click a cell for the player card, an unknown cell to name him) — it is the very component the mock board renders, so screen sync is tested on the pixels the product shows. To try it without a room, open **/mock-board** in another tab (a DraftKings look-alike that drafts on a timer; `?overall=0` hides the gray overall pick, `?zoom=1.7` shows ~7 columns) and share that tab. Dark-mode rooms are inverted and upscaled before OCR; "what the OCR read" shows the raw lines so you can tune the box. The OCR engine downloads on first use (a few MB, from Tesseract's free CDN) and is cached after.
- **Paste picks** (any site, one paste). Copy the drafted list from the draft room, press ⌘V anywhere in the cockpit (or click **Paste picks**), and the preview shows what was recognized, what wasn't (with did-you-mean chips) and what's already off the board. It understands Sleeper `1.05 Name RB - ATL`, ESPN's live room (`Puka Nacua / LAR WR` with `R1, P2 - Team 7` underneath), ESPN's recap `1 (1) Name, ATL RB`, Yahoo `1. (1) Name (Atl - RB)`, `Last, First`, initials, pick/name/meta on separate lines, several names on one line, and bare numbered lists. **Paste backfills what the screen missed.** With pick numbers, each name lands at its number: a placeholder is filled, a gap is padded with unknown picks, and a different player already sitting at that number means a pick was missed — the pasted one is inserted in front and the later picks move down to their real slots. Without pick numbers, the list is aligned by order with the picks already on the board (the same reconciliation screen sync uses), so a name between two known picks slots in between them. Names already on the board are never re-marked; they are the anchors. A toggle handles newest-first lists. **Copying a draft board grid** (DraftKings) yields cells row by row in screen order — but even rounds run right to left, so order is useless; the cells' `6.12` / `7.1` labels (single-digit picks included) are read as pick numbers instead, whether each field is on its own line or a row came through tab-separated. If the copy carries no labels at all, the cockpit lays the names onto the board from what it already knows (`lib/draft/pasteLayout.ts`): a draft fills picks in order, so the count of recorded picks says where the new ones start, every pasted name that is already on the board pins its pick and column, and the draft order says how rows snake — paste any subsection of the grid, even one that turns the corner from pick 12 to 13, and it lands right. Board copies (cells with "(BYE 7)" tags) are read in screen order, lists chronologically, newest-first lists from their anchors; when two readings both fit the preview says which it chose and offers the other in one click.
...
**Newsroom** (`/newsroom`, linked from Setup and the Cockpit's ⋯ menu): every player and every update without starting a draft. The top feed ranks updates by *what matters* — severity of the news (season-ending 1.0 · suspension 0.9 · out 0.7 · transaction 0.6 · doubtful 0.5 · cleared 0.4 · questionable 0.35 · depth chart 0.3 · mention 0.1) × ADP relevance (1.0 at the top of the board, 0.2 from ADP 300) × recency (half-life 12 h) — so an ADP-50 torn ACL outranks an ADP-1 practice note. Status changes the live ESPN table made versus the built board appear as their own items. **Top stories** puts the five most important fresh updates (one per player, at least Questionable-grade) on position-colored cards with the player's ESPN headshot and team mark. **The wire** below carries every update with a photo, a kind pill, the source and a link to the original post; sort by relevance, newest, oldest or ADP, and hone in with the Filters menu (kinds, teams, and per-source show-only/hide, including individual Bluesky accounts — remembered on-device). Below it, the full board is browsable and sortable with the same player cards the Cockpit uses. New items never reorder the list while you are scrolled down; they wait behind an "N new" pill.

**Source toggles** (Setup → Advanced → Data sources): pick ESPN, Sleeper, or a blend for projections; FFC, Sleeper, ESPN, or a blend for ADP. Blended projections are the default — two independent models beat either alone. FFC stays the uncertainty model regardless: it's the only free source publishing per-player ADP spread, which powers the survival math. The one optional key is FantasyPros (a GitHub secret, build-time only). Its free tier allows roughly ten requests a day at ten players each, so the daily lane spends them deliberately: the news endpoint first, then PPR consensus ranks for the top 60 (2QB shares them); standard and half-PPR keep the DynastyProcess weekly consensus. A paid key can raise `FANTASYPROS_DAILY_BUDGET` to add projections. Everything else keyed/paid (FantasyData, Fantasy Nerds) is deliberately excluded.
