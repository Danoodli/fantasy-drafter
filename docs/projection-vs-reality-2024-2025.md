...
**Do we beat the crowd?** Yes, in both years — after fixing a bug this test exposed. Against a drafter who just takes the next player by ADP — and who, like a real manager, streams a waiver pickup into any empty slot — the engine's roster finished about **5th of 12 on average in 2024 and 3rd of 12 in 2025**, and out-scored the ADP drafter sitting in the same seat about 65% of the time in 2024 and 85% in 2025. (Without the waiver wire in the scoring the edge looks roughly three times bigger; that version flatters the engine by punishing the bot's empty slots with zeros nobody would actually take.) Before the fix, it *lost* to the ADP drafter in 2024 (finished ~9th of 12), because it was drafting 7 running backs and 2 wide receivers — which you saw yourself in your Sleeper mock. That was a math artifact, not a football opinion, and it's gone.
...
| `balanced`, redraft PPR, 12×15 | 2024 | 2025 | roster shape |
|---|---|---|---|
| **old value model** (league-wide VORP/VOLS) | **−111** ± 27 · avg finish 8.6 of 12 · 1% first | +305 ± 31 · avg finish 2.2 · 52% first | 6.8 RB / 2.2 WR |
| lineup-marginal starters, VORP bench | +90 to +113 ± 25 · avg finish 4.0–4.7 | +255 to +312 ± 25 · avg finish 2.0–2.5 | 5.6 RB / 3.4 WR |
| + insurance bench, floors | +120 ± 25 · avg finish 4.3 · 19% first | +305 ± 29 · avg finish 2.1 · 54% first | 4.9 RB / 4.1 WR · 0 floor violations |
| **same, scored with a waiver wire** (shipped; honest yardstick) | **+48** ± 18 · avg finish 5.1 · 11% first | **+140** ± 18 · avg finish 3.0 · 35% first | **5.1 RB / 4.0 WR** (2024), 4.3 / 4.7 (2025) · 0 violations |

The old model lost to a plain ADP bot in 2024 because it drafted **6.8 RBs and 2.2 WRs** into a 2-RB/2-WR/FLEX lineup (your own Sleeper mock came out 7/2). Root cause: it valued every player against league-wide replacement level, and RB58 projects ~80 points while WR58 projects ~161, so two players with identical projections headed for the same FLEX slot differed by ~84 value points purely by position. That is a baseline artifact, not a football insight. The new model values a *starter* by what he adds to your lineup over the best you can expect at that position at your next pick. The first version still valued the *bench* by VORP, which left a 5.6 RB / 3.4 WR lean and, in a real Sleeper room, produced another 7 RB / 2 WR roster — expected points never saw the problem, because a roster that can't field its lineup only loses the points it fails to score. Bench players are now valued as **insurance**: the expected starting slot-weeks they fill that the roster would otherwise leave empty (byes are certain; injuries by position rate) times their scoring rate. That, plus a 2 QB / 3 RB / 3 WR backstop, produces the shipped shape above, with the engine's rosters *more* robust than the ADP crowd's (3.9–4.1 expected empty slot-weeks per season vs 6.4–7.0). Every one of the nine strategies flipped from negative to positive in 2024 under the lineup model, and 2025 held.

*Update (2026-09-05):* the yardstick was tightened further — redraft lineups are chosen before kickoff and the wire is the 3rd-best undrafted player *by projection* with a friction cost — and the shipped model measures **+110 ± 16 (2024) / +213 ± 25 (2025)** under it (the bot loses more to ex-ante lineups than the engine does). A unified decision model built on a calibrated outcome model was tested against it and tied in 2024 (+99 ± 13) but lost 2025 and best ball; it is available behind `valueModel: "unified"` and its gate results are in `docs/backtest-gates.md`.

The last row is the one to believe. Adding a waiver wire to the *scoring* (one streamable body per position per week, drawn from that room's actual undrafted players) roughly triples the ADP bot's score at its empty slots and shrinks the engine's edge from +120/+305 to **+48/+140**. That is not the engine getting worse — it is the yardstick stopping to reward depth that a real manager would replace from free agency. The engine is still ahead in both years at 2σ, its rosters are far more robust (0% below a floor vs 24–27%), and 2024's modest edge remains the realistic expectation.
